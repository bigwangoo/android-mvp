...
/**
 * 用于管理所有 activity
 * 在前台的 activity可以通过直接持有 AppManager 对象执行对应方法
 * 也可以通过 {@link #post(Message)} ,远程遥控执行对应方法,用法和 EventBus 类似
 * <p>
 * Created in 2017/9/21 9:22.
 *
 * @author Wang YaoDong.
 */
@Singleton
public final class AppManager {
    ...
    public static final String MESSAGE_APP_MANAGER = "message_app_manager";
    // true 为不需要加入到 Activity 容器进行统一管理,默认为 false
    public static final String IS_NOT_ADD_ACTIVITY_LIST = "is_not_add_activity_list";

    public static final int TYPE_START_ACTIVITY = 5000;
    public static final int TYPE_SHOW_SNACK_BAR = 5001;
    public static final int TYPE_KILL_ALL = 5002;
    public static final int TYPE_APP_EXIT = 5003;
    ...
    public static void post(Message msg) {
        EventBus.getDefault().post(msg, MESSAGE_APP_MANAGER);
    }

    /**
     * 通过 EventBus post 事件,远程遥控执行对应方法
     */
    @Subscriber(tag = MESSAGE_APP_MANAGER, mode = ThreadMode.MAIN)
    ...
    /**
     * 让在前台的 activity,使用snackBar显示文本内容
     *
     * @param message
     * @param isLong
     */
     ...
     * 是在 onStop 之前,所以如果当此 activity 调用 onStop 方法之后,没有其他的 activity
     * 回到前台(用户返回桌面或者打开了其他 App 会出现此状况)
     * 这时调用 {@link #getCurrentActivity()} 有可能返回 null,所以请注意使用场景和
     * {@link #getTopActivity()} 不一样
     * <p>
     * Example usage:
     * 使用场景比较适合,只需要在可见状态的 activity 上执行的操作
     * 如当后台 service 执行某个任务时,需要让前台 activity ,做出某种响应操作或其他操作,如弹出 Dialog,
     * 这时在 service 中就可以使用 {@link #getCurrentActivity()}
     * 如果返回为 null ,说明没有前台 activity (用户返回桌面或者打开了其他 App 会出现此状况),
     * 则不做任何操作,不为 null ,则弹出 Dialog
     *
     * @return
     */
    ...
    /**
     * 将在前台的 activity 赋值给 currentActivity,注意此方法是在 onResume 方法执行时将栈顶的
     * activity 赋值给 currentActivity
     * 所以在栈顶的 activity 执行 onCreate 方法时使用 {@link #getCurrentActivity()}
     * 获取的就不是当前栈顶的 activity,可能是上一个 activity
     * 如果在 App 的第一个 activity 执行 onCreate 方法时使用 {@link #getCurrentActivity()}
     * 则会出现返回为 null 的情况
     ...
     *
     * @param excludeActivityClasses class
     */
    ...
    public void appExit() {
        try {
            ...
            am.killBackgroundProcesses(mApplication.getPackageName());
            System.exit(0);
        } catch (Exception e) {
            e.printStackTrace();
        }
