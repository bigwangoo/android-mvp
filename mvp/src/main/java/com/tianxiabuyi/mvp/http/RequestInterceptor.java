...
import com.tianxiabuyi.mvp.utils.CharacterUtils;
import com.tianxiabuyi.mvp.utils.ZipHelper;
        ...
        if (isParseable(responseBody.contentType())) {
            ...
            if (logResponse) {
                Timber.tag(getTag(request, "Response_Result")).w(isJson(responseBody.contentType()) ?
                        CharacterUtils.jsonFormat(bodyString) : isXml(responseBody.contentType()) ?
                        CharacterUtils.xmlFormat(bodyString) : bodyString);
            }
        } else {
            if (logResponse) {
